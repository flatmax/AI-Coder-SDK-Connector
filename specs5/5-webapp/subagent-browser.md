...
- **New turn** — the previous turn's subagent tabs leave the strip. Their transcripts remain on disk under the session's `subagents/` directory and stay reachable through the history browser.
  ...
  **Except a turn whose background subagents are still running.** Sending while they work is what background agents are for, so the send *parks* that turn (`parkBackgroundTurn` in `streaming.js`) instead of overwriting it: its request id and block state move to the tab's `backgroundTurns`, and its subagents' tabs stay in the strip. The engine routes each message to the turn that owns it ([`../3-engine/session.md`](../3-engine/session.md) § Every message goes to the turn that owns it), so the older subagents' blocks keep arriving under the older request id, and the browser folds them into the parked turn and mirrors them into those tabs — never into the running turn. When the parked turn's last result arrives it revises that turn's settled message where it sits in the transcript, and the running turn's spinner, timer and edit LED are left alone. The tabs then leave with the next send after that, as usual. Numbering continues from the highest ordinal still in the strip, so a surviving `1` is never joined by a second one.

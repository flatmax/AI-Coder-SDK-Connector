...
### Card Anatomy
- Header — tool name, a one-line input summary (≤ 200 chars), and a status dot. MCP tools show their server name (`aic-dc`) as a chip so a user can tell an AIC⚡DC tool from a built-in one
...
- **The header's input summary is the one place a raw absolute path still shows**, as `file_path=/home/you/repo/…`, and that is deliberate rather than missed. The summary is built server-side as a `key=value` join over whatever keys the input has, so it does not know which of them are paths; teaching it would mean a per-tool table of path keys — a *third* mechanism answering "absolute → relative" when there are already two ([`../plan/README.md`](../plan/README.md) open item 3). It also truncates from the front, so the cap falls on the tail. Worth fixing with that convergence, not before it

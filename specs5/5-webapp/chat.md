...
### Card Anatomy
- Header — tool name, an input summary (≤ 200 chars), and a status dot. MCP tools show their server name (`aic-dc`) as a chip so a user can tell an AIC⚡DC tool from a built-in one
...
- **The engine's 200-character cap is the only bound; there is no line clamp on top of it.** A clamp is the same ellipsis three rows lower, and the cap already bounds how tall a card can get. What replaces it is a header that **wraps as a row**: the summary carries a `10rem` flex basis and the gated marker, time chip and caret travel as one right-hand group, so a pane too narrow to give the summary that basis puts it on a line of its own instead of squeezing it to a column of single broken characters. That failure is reachable — `DIALOG_MIN_WIDTH` is 300px — and it is invisible to the suite, because jsdom does no layout; the three widths were read in a browser
...
- **The header's input summary is the one place a raw absolute path still shows**, as `file_path=/home/you/repo/src/a.js`, and that is deliberate rather than missed. The summary is built server-side as a `key=value` join over whatever keys the input has, so it does not know which of them are paths; teaching it would mean a per-tool table of path keys — a *third* mechanism answering "absolute → relative" when there are already two ([`../plan/README.md`](../plan/README.md) open item 3). It also truncates from the front, so the cap falls on the tail. Worth fixing with that convergence, not before it — and now **worth more**, because the wrapping header shows the whole prefix instead of clipping it: the two or three rows a nested absolute path costs are rows spent on a string identical for every file in the repo

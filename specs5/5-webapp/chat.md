...
| `rateLimit` | Warning toast on `allowed_warning`, error toast on `rejected`, both naming the reset time and the window in the same words the HUD's section uses (`rate-limit.js`), so one limit is not named two ways. `allowed` says nothing — the standing figure is the HUD's, and this is the transition |

...
- **File mention** — a path typed in the prompt as `@path`. The **CLI** expands it and reads the file; AC⚡DC's part is the `@`-triggered path filter in the chat input, and the picker's shift+middle-click, which inserts the `@path` form. This is the whole of "pointing at a file" now.

...
| Put the breach where a skimming reader will see it | [AG-R-29](risks.md#ag-r-29) | Two small changes in `bridge.py`, both for the human reading a **restored** session rather than the model reading the result. Hoist `_grounding`'s retraction above the "A second opinion from Google Antigravity …" boilerplate when `escaped` or `unknown` is non-empty, so a previewed or truncated result leads with the withdrawal instead of the assurance; and set `isError` on the returned result when `escaped` is non-empty, so the collapsed card carries a warning badge rather than the neutral completed styling it shares with a consultation whose gate held |

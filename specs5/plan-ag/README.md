...
| **12. The stop is a mechanism** ⬜ | [AG-19](decisions.md#ag-19). A `PostInvocation` handler on the existing gate socket returning `terminationBehavior: "terminate"` while ⏹ is latched, so a stop ends the loop whatever the agent concludes; `PreToolUse` denial retained as the tree's protection; process kill demoted to an explicit user escalation. Measured before being specified — 4 invocations become 1, the conversation stays usable afterwards, and `SIGINT` is fatal while a respawn costs 3.7s. | A turn that ignores its refusals still ends, and a turn ended this way renders as **cancelled** rather than as a successful blank answer. The residual gap — prose inside one invocation — is documented rather than closed. |

## Unbuilt on the `agy` transport, as of 2026-09-10

**Everything below is specified and none of it is written.** Recorded as a list because the table
...
| Read `transcriptPath` from the hook payload | same § *Four details the probe did not have* | `subagents.py` derives it by hand today; the payload carries it, and carries the per-product directory name that differs |

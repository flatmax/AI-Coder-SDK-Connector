...
**Two findings came with it, both in the config layer and both about AG-17's premise:** `app.json` is
a *managed* file, so the first start after any version change silently reverts `engines.enabled` and a
Claude-only install is a two-provider install again; and shipping it read-only — which AG-17 already
recommends — does hold the policy, at the cost of an upgrade pass that aborts and repeats on every
start. Neither is fixed here, because both change the config layer's upgrade contract for every key in
that file. See [`delivery.md` § Phase 11](delivery.md#phase-11--the-engine-policy-and-what-a-fresh-server-said-about-it-2026-09-07).

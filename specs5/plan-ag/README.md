...
| Stop creating a consultation's strip tab eagerly | [AG-29](decisions.md#ag-29) | The remaining half of step 3's surface work, and it is now unblocked rather than done: the notices no longer *need* a mounted tab to have somewhere to be, since the posture is framing and the store is keyed on `agent_id`. What is left is to create the tab on demand from an affordance on the inline card, so the strip stops filling with tabs competing for attention with the stream the reader is actually in. Now unblocked on both counts: the projection it depends on is built ([AG-30](decisions.md#ag-30)), and `_projectSubagentBlocks` is the function an on-demand tab mounts from |

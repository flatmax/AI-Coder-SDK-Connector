...
| An authenticated HTTP MCP listener, and a per-spawn `mcp_config.json` in the private root | [AG-22](decisions.md#ag-22) | A second listener on `127.0.0.1:0`, a `token → {repo_root, session_id}` map, and the config written at spawn. This is what makes `claude` reachable as a **consultant** and closes the half of [AG-1](decisions.md#ag-1) that has never existed. Needed [R-14](../plan/risks.md#r-14--two-refreshes-race-for-one-single-use-refresh-token)'s lock first; **that shipped 2026-09-11**, so nothing is in front of this any more |

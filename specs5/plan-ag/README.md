...
| **12. The stop is a mechanism** ✅ | [AG-19](decisions.md#ag-19). A `PostInvocation` handler on the existing gate socket returning `terminationBehavior: "terminate"` while ⏹ is latched, so a stop ends the loop whatever the agent concludes; `PreToolUse` denial retained as the tree's protection; process kill demoted to an explicit user escalation. Measured before being specified — 4 invocations become 1, the conversation stays usable afterwards, and `SIGINT` is fatal while a respawn costs 3.7s. [AG-R-16](risks.md#ag-r-16)'s `Stop` handler ships with it, on the same registration. | A turn that ignores its refusals still ends, and a turn ended this way renders as **cancelled** rather than as a successful blank answer. The residual gap — prose inside one invocation — is documented rather than closed, and its presentational handling was **built 2026-09-12**: the view freezes at the latch, the usage keeps accumulating, `agy`'s finished prose is refused so it cannot reappear at the footer, and a `stop_acknowledged` card says the screen is final. **Built 2026-09-11** — three handlers under one hook name, the event stamped from argv rather than read off the payload, each command probed before it is written, and 84 tests. **Met the same day**, by `scripts/probe_agy_stop_terminates.py` against an isolated `--gemini_dir`: **the armed run ends in 1 invocation where the control needs 3**, on the same prompt stopped at the same point, with the *only* difference being whether the host answers `terminate` or `{}`. A stopped turn ending proves nothing on its own — a cooperative agent ends one too — so the control is the instrument, and the probe reports **inconclusive** rather than pass when the control also ends immediately. Eight checks held: the armed loop ends by `TERMINAL_CUSTOM_HOOK` and the control by `NO_TOOL_CALL`, both footers carry `cancelled`, and every call after the stop is denied on *both* runs, so the termination is a second layer rather than a replacement. The probe failed its own control first — the recorder discarded a real decision and left `was_terminated` claiming a termination it never sent. One existing claim was falsified on the way: `workspacePaths` is **populated** on all three events, not empty. See [`delivery.md` § Phase 12](delivery.md#phase-12--the-stop-becomes-a-mechanism-and-the-control-is-the-instrument-2026-09-11). |

## Unbuilt on the `agy` transport, as of 2026-09-10

**Everything below is specified and none of it is written** *(as of 2026-09-10; three have since
been built — see the note under this paragraph)*. Recorded as a list because the table
...
**[AG-19](decisions.md#ag-19)'s presentational half followed on the same day.** Its mechanism shipped
on 2026-09-11 with the residual gap it had named — a prose answer runs to the end of its own
invocation — and the specification's handling for that gap, *stop updating the view*, turned out never
to have been built: the pump was told about the stop only after the frame loop, when there was
nothing left to suppress, so **a stopped prose turn streamed its whole answer and then said it had
been stopped**. The view now freezes at the latch while the meter keeps running and the stream drains
into the warm process, and a `stop_acknowledged` card says the screen is final, because a freeze
...
**Six rows joined the list on 2026-09-11.** Three came from
...
**One of the six is specified and unwritten; the other five all landed on 2026-09-11 — and the build added a seventh, [AG-R-23](risks.md#ag-r-23).** The reconnect
...
**Three more left the list on 2026-09-12, and one arrived — the listener itself.** Migration step 4
is built and tested but reaches nothing yet, so the row was **split**: the server is struck through and
its wiring into `AgySession` is a row of its own, because a listener nothing can call closes
[AG-1](decisions.md#ag-1)'s asymmetry only on paper. The quota is ◑ rather than ✅ for a clause the
mechanism cannot express — a bearer token names a session, not which agent inside it is asking. Two of
the day's findings are worth a reader's time before they touch this code: the SDK's MCP session table
is reclaimed in **exactly one place and it is not the one that runs**, so *the tidy path is the leaking
one* ([AG-R-25](risks.md#ag-r-25)); and a cancelled `async` branch that awaits anything does nothing at
all, which is how the first cut of the stop shipped a hole its tests could not see
([AG-26](decisions.md#ag-26)). See
[§ The listener, and the two ways a cancellation can be silently absorbed](delivery.md#the-listener-and-the-two-ways-a-cancellation-can-be-silently-absorbed-2026-09-12).

**Later the same day the wiring landed, and the split row closed with it.** The listener had 26 tests,
four probes and a decision entry arguing over its cancellation semantics, and the application had never
started one — so the asymmetry was closed on paper and nowhere else. It is now closed on the wire:
`agy`, on the user's subscription, reaches Claude through this host, and the canary sentence comes back
byte for byte. The quota's ◑ is ✅ as well, its last clause having been measured buildable rather than
assumed otherwise. Two findings from the wiring are worth a reader's time: the permission dialog would
have granted **every MCP server at once** without a new match kind, because `agy` reaches all of them
through one multiplexed tool ([AG-27](decisions.md#ag-27)); and the delegate refusal it relies on is
**policy against `agy`'s own dispatcher, not a boundary** — anyone holding the bearer can omit the
`_meta` that identifies a subagent, and the three reasons that is accepted rather than fixed are in
[AG-R-26](risks.md#ag-r-26). See
[§ The consultant that was built, tested, measured, and never started](delivery.md#the-consultant-that-was-built-tested-measured-and-never-started-2026-09-12).

The consultation thread that shaped all of this **converged at round fifteen** — *"we are done, ship the
design as it stands"* — and then ran two rounds further on the one claim it would not grant, closing at
seventeen. One reversal, one conceded limit and one held objection, all recorded in
[§ The round that ended it](delivery.md#the-round-that-ended-it-and-the-one-thing-it-would-not-concede-2026-09-12).
The objection was the useful one: the 5.6-second stop latency is confounded, because the latched gate
starves the master's next tool call regardless, so `agy` would have wound up at about that time even with
a consultation still running and still spending the subscription. It is closed now by artefacts rather
than by a returned boolean — the scratch directory and the `claude` CLI child are both gone 5.8 seconds
after the stop, and the same artefacts are now a regression test that needs no `agy` at all
([AG-27](decisions.md#ag-27)). What a stop cannot do is recall a request already on the wire, which is
recorded as [AG-R-27](risks.md#ag-r-27) and bounded to one turn rather than mitigated.

**One row left on 2026-09-14, and it is the only one on this list that was a shipped defect rather than
unbuilt work** — ⏹ Stop on a consultation row reached the consultant with the row's id thrown away, so it
stopped whichever consultation had started most recently. Two of its three faults were found while
building it rather than specified: a shared cancellation flag that made a *finished* consultation report
itself stopped, and a shared connection handle that made a failing one quote a working harness's stderr.
Both are the same mistake as the slot the row named — one piece of state per consultant where the state
belongs to a consultation — which is the argument for looking at the neighbours of anything on this list
before calling it fixed. It closes migration step 3. See
[§ The stop that was aimed and then unaimed](delivery.md#the-stop-that-was-aimed-and-then-unaimed-2026-09-14).

**Two more went later the same day, and the paragraph above needs correcting: the shipped defect was
not the only one.** Rows 691 and 692 read as tidying — move the write guidance onto the vendor's own
channel, read a path off the payload instead of deriving it — and the guidance row's real content was
that the shipped channel was *already failing* in two ways nobody had written down. It arrived **once
per turn** on a transport where a turn is many invocations, so every invocation after the first ran
unguided; and it reached the **master only**, so no subagent had ever been told. The row named a third
fault, the mirror storing words as the user's that the user had not written, and that is the one that
was cosmetic. Delivery of the replacement was measured twice rather than argued: four injections at
`invocationNum [0,1,2,3]`, then a second probe mode after the model's own account of where the message
came from turned out to be wrong. Row 692's own surprise was in the neighbours again — the transcript
reader resolved its directory *twice*, once to check that an id belonged to this session and once to
read it, so two candidate directories could have approved an id against one store and returned a
transcript from the other. What is left open is [AG-R-32](risks.md#ag-r-32): two readers still derive
`roots.PRODUCT_DIR`, and both fail by reporting nothing. See
[§ The guidance that stopped pretending the user had typed it](delivery.md#the-guidance-that-stopped-pretending-the-user-had-typed-it-2026-09-14).

**A third went the same day, and it corrects that correction: the shipped defect was not the only one
either.** Row 709 was not on the list at all — it was found while checking the neighbours of row 691,
in the one place a reader would not look, a docstring that already claimed the feature worked. The
browser has pushed the open file to whichever adapter is master since phase 3, and both Antigravity
adapters **stored it and never read it**, so the file on the user's screen reached the model on one
engine of three. The interesting part is not the missing read. It is that the fix's channel had to be
argued *against* row 691, decided hours earlier: [AG-32](decisions.md#ag-32) took a message off the
prompt for three faults, and [AG-33](decisions.md#ag-33) put a different message back on it, because a
standing instruction and a turn-scoped fact have different lifetimes and the channel follows the
lifetime rather than the precedent. Where the work went was the neighbours again — `set_viewer_state`
had been written twice with **no test on either engine**, so the two copies had drifted apart in three
ways that were all harmless for exactly as long as nothing read them. Left open was
[AG-R-33](risks.md#ag-r-33): the Antigravity model has no `ui_state` to ask with, so on those transports
the turn's framing is the only report there is. See
[§ The file on the user's screen, on every engine](delivery.md#the-file-on-the-users-screen-on-every-engine-2026-09-14).

**And that residue was six times larger than it was written up as.** Measuring it the next day —
row 725's own lesson, one day old — found that `claude_code/mcp_server.py` has exactly one importer, so
**none** of the six `aic-dc` tools reached either Antigravity transport and none ever had.
[AG-34](decisions.md#ag-34) is that build, and what it cost was not the snapshot: it was the gate policy
AG-R-33 correctly predicted, plus two things it did not — the SDK derives a bare callable's schema from
its *signature*, and `deny_all()` would have refused the six custom tools after showing them to the
model. Closed as [AG-R-33](risks.md#ag-r-33); the one clause left wrong is now
[AG-R-34](risks.md#ag-r-34).
...
| ~~Standing guidance via `PreInvocation` `ephemeralMessage`, re-injected every invocation~~ ✅ | [AG-32](decisions.md#ag-32), [`sdk-surface.md` § There are five lifecycle events](sdk-surface.md#there-are-five-lifecycle-events-and-this-app-wires-four) | **Built 2026-09-14.** `AgyGateServer.standing_guidance` answers the sentence on every `invocationNum`, `hook.inject_guidance` wraps it, and `chat_streaming` passes the user's text through untouched. The row named one fault and there were **three**, all of them about the channel and none about the words: the mirror stored a paragraph as the user's that the user had not written; the guidance arrived **once per turn**, where the write it guards against can happen at any invocation; and it reached the **master only**, so a subagent — which never sees the parent's prompt — was never told at all. **The frame is built in the hook rather than forwarded from the socket**, because `injectSteps` accepts `{"toolCall": …}` beside the ephemeral message: a host bug that could put its answer straight into `agy`'s step list could put a tool call there, and that call never passes the gate, which is installed for calls the *model* emits. So the socket carries prose and nothing else — a test hands back a correctly-shaped `injectSteps` and it is deliberately rejected. **Delivery was measured twice, not assumed.** A multi-step probe took 4 injections at `invocationNum [0,1,2,3]` with a nonce in the armed reply and none in the control's, which is why `standing_guidance` holds no "already sent" latch. That model then claimed the rule had arrived *appended to tool output* and not before it began — which would have meant the first tool call of every turn was unguided — so `scripts/probe_agy_pre_invocation.py` gained a `--first-invocation` mode: one invocation, no tools, and the armed reply **opened with the nonce** and quoted the rule. There is no gap, and the model's account of its own context provenance was simply wrong — [AG-16](decisions.md#ag-16)'s *a model's introspective report about its own bias is not evidence*, arriving for a different question and costing nothing because the claim was checkable. A blank-but-truthy guidance string was found injecting `{"ephemeralMessage": ""}` by its own parametrised test — a step that says nothing, from a caller who meant *no guidance*. See [§ The guidance that stopped pretending the user had typed it](delivery.md#the-guidance-that-stopped-pretending-the-user-had-typed-it-2026-09-14) |
| ~~Read `transcriptPath` from the hook payload~~ ✅ | [AG-32](decisions.md#ag-32), [AG-R-32](risks.md#ag-r-32), same § *Four details the probe did not have* | **Built 2026-09-14 with the row above**, because both are the same hook events learning to carry something. `AgyGateServer.note_paths` inverts `transcriptPath` and `artifactDirectoryPath` on **every** event rather than on tool calls only, so a turn that writes nothing but prose still answers the question; `roots.brain_dir` is **kept behind** the announced answer rather than replaced by it, since the derivation is what answers for a session this process never ran. A **per-conversation** map of announced paths was rejected: one `agy` process has one `HOME` and therefore one brain dir, and `subagents.rows` computes `path.relative_to(brain_dir)`, which a per-id path outside it would raise on. The fault the row did not contain is what most of the work went into: `get_subagent_transcript` checks containment with `descendants` and then reads with `load`, and resolving the directory *independently* in each would let a machine where both candidates exist approve an id against one store and hand back a transcript from the other. `subagents.choose_brain_dir` picks **one** directory per request and both reads use it; the test stages an impostor child in the announced store and asserts the answer comes from there. Two smaller things measured while building: the inversion must take the **nearest** enclosing match, or a conversation id that also names a parent directory resolves the brain dir too high; and the disagreement warning fires **once**, because after that it is the same fact repeating. See [§ The guidance that stopped pretending the user had typed it](delivery.md#the-guidance-that-stopped-pretending-the-user-had-typed-it-2026-09-14) |
| ~~Frame what the user is looking at on every engine, from one module~~ ✅ | [AG-33](decisions.md#ag-33), [AG-R-33](risks.md#ag-r-33) | **Built 2026-09-14, third of three that day, and the scope was wrong in one direction.** Written up as needing *"a `set_viewer_state` on `AgyService`"*; measured first, and `AgyService` already had one by inheritance, `set_viewer_state` is not in `RPC_SURFACES` so the router mounts it as a core method, and the browser's push had been arriving since phase 3. **No new RPC method and no webapp change.** The missing half was the *read*: `_viewer` was assigned by three paths across the two adapters and consulted by none, so opening a file and asking *"why is this failing?"* worked on one engine of three — [AG-1](decisions.md#ag-1)'s asymmetry, shipped, with the adapter's own docstring promising *"for turn framing"* since the day it was written. The channel is the **prompt**, decided against a naive reading of [AG-32](decisions.md#ag-32) hours earlier: of its three faults only *stored as the user's words* transfers, because a viewer is turn-scoped where guidance is standing, and re-asserting it per invocation would either repeat a stale claim as live or contradict the turn's own opening framing. New `src/aic_dc/framing.py` owns the wrapper, validator, sentences, composition and precedence; `ViewerFraming` is an alias, since it is a published name in `aic_dc.claude_code.__all__`, and `history.py` imports the tags it used to spell itself — they were in **three** places, two of them a renderer and its reader. Where the work actually went: `set_viewer_state` had been written twice with **no test on either engine**, so the copies had silently diverged — `{}` versus `None` for empty, `start_line: None` stored verbatim where Claude omitted it (one push from printing `(cursor on line None)` at the model), and no type validation at all, so `set_viewer_state(42)` stored `{"path": 42}`. And `if viewer:` meant a closed pane could not clear a stale push, so the agent stayed pointed at a file nobody was looking at. Left open: [AG-R-33](risks.md#ag-r-33), no `ui_state` for the Antigravity model to ask with. See [§ The file on the user's screen, on every engine](delivery.md#the-file-on-the-users-screen-on-every-engine-2026-09-14) |
...
| ~~Detect a renamed control tool, instead of denying the consultant its own `finish`~~ ✅ | [AG-R-22](risks.md#ag-r-22) | **Built 2026-09-14, and the row's own question — what to do about it — is answered *report, never permit*.** The detection is not the gate seeing a name it denies, as the row assumed: it is `agy`'s **`init` frame**, which carries the whole tool inventory by name and arrives **before any prompt is sent**, so the check costs no model turn and no subscription spend and can therefore run on every consultation rather than in a probe somebody must remember to re-run. `AgySession` keeps it as `advertised_tools`; `_missing_from_binary(policy, advertised)` is `policy.allowed - advertised`, generalised over the policy so a renamed `generate_image` is diagnosed exactly as a renamed `finish` is; the fact is recorded on the translator as a **fourth list** — `unadvertised_tools`, beside `ungrounded`/`breached`/`unverified` — so the timeout message and the empty-answer message read one source and cannot disagree. **Nothing is permitted on the strength of it**, which is the row's objection honoured rather than worked around, and **no launch is refused on it**, which is measurement rather than caution: AG-R-22's own P24 arm D answered with no tool call at all, so a missing `finish` makes failure likely and not certain, and refusing would trade a diagnosable failure for a guaranteed one. **An empty inventory means *no claim*, never *no tools*** — an `init` frame with no list, from an older binary, must not make every consultation announce that everything it permits has gone missing. Measured while building: `agy` is **1.2.2** here (the specs say 1.2.0), advertises **57** tools with `finish` among them and `unclassified: []`, and has **no** tool-listing subcommand — the `init` frame is the only free oracle there is. 9 tests here and 3 on the session, run against the shipped source first: the timeout said only *"Antigravity did not answer within 2s"*. See [§ The two silences at the gate](delivery.md#the-two-silences-at-the-gate-2026-09-14) |
| Keep an ephemeral consultation root when the consultation **failed** | [AG-R-23](risks.md#ag-r-23) | One condition in `roots.ephemeral`'s `finally`, plus the path at `warning`. [AG-21](decisions.md#ag-21) deletes the root whether the consultation worked or not, so the only case where anyone wants the vendor's tree is the case where it is already gone. Found by checking what AG-21 did to `scripts/`, not by a test — the suite cannot see it, because every test that would care builds its own root and keeps it |
| ~~Render a denied tool call in the consultation tab~~ ✅ | [AG-R-22](risks.md#ag-r-22) | **Built 2026-09-12, and the row was wrong about what it was.** Specified as presentational — "the hook already reports the deny" — on the premise that nothing had been written to show it. In fact `agy` reports failures in `tool_info.error` and never in `tool_info.output`, and `AgyTranslator` read only `output`: the gate's own sentence arrived in the frame and was discarded one line from the browser, so **every denial and every tool error drew the literal "No output."** — measured on both surfaces, a consultation denied `read_url_content` and the **master engine** denied `view_file`, which the row did not cover at all. Four parts: the card renders the error; a refusal is told from a failure by the **conjunction** of "off the consultation's allowlist" and "carries `StaticPolicy.MARK`", so a hallucinated tool name or a gate that failed open and then failed on its own is never reported as containment that held; the ungrounded notice fires at the **first** refusal rather than at the `result` frame a stopped or crashed consultation never sends, scoped to the consultation's tab; and the model that asked is handed a header naming every refused tool, above an answer quoted between markers carrying a **per-call nonce** — reading order alone was forgeable, since the answer passes through verbatim. **Reworked again 2026-09-12 after a third review round**, which found the mark answering the wrong question: keyed on it, the grounding warning fired only when *this app* was what refused the call. P24 arm C measured the hole — `agy` rejected `read_url_content` against the tool's own schema in 203ms without running our hook, so an unmarked failure took a retrieval out of the answer and nothing said so. The notice now keys on the **outcome** (off the allowlist and failed, whoever stopped it) and the mark keeps only the narrower job of settling the card on our authority. The bridge header states the **no-tools posture unconditionally**, because the consultation that never tries — P24 arm D, 45 events and no tool call — is the one that answers from its weights with nothing to warn the caller; and the notice **names no tool**, since a list raised once goes stale the moment the next refused card renders. **And once more after a fourth round**, which found the mark's last job leaking the same bug: a barred call that failed *without* our mark — arm C's shape — carried an error and no terminal state, so its card kept a pending spinner under a finished consultation. The mark is now out of detection entirely; one predicate, `_is_barred`, answers both *is the call over* and *did the answer lose a retrieval* from the allowlist alone, and the mark is only the sentence a person reads on the card. The tab gained the header's unconditional posture as a `consultation_posture` row pushed before any output, toast-free because it is a standing condition rather than an event. **And a third time after a fifth round, which broke the pattern the first four had set**: every earlier hole was the app being *silent*, and these are the app being *wrong*. `failed = state == "ERROR" or stopped` asked the vendor whether a call this app knew could not run had gone well, so a release reporting a denial as `DONE` with the reason in `output` would have drawn a **green success card** and silenced the notice and the header with it; a barred call that settles now fails whatever word `agy` put on it. `_is_barred` required a name, so a tool step whose name the pump could not read was *permitted* — dropped, and a nameless call reaches the asking model as "a tool it did not name". The reviewer's "never let a barred tool succeed" is adopted with a hole cut in it: a barred tool that returns output and no error **ran**, and filing that as a refusal would hide the one event that makes the header false, so it raises `consultation_breach` and the bridge *withdraws* the posture. And a card the consultation never reported is closed by `settle_pending` — called from the bridge's tab teardown as well as the pump's footer, because a consultation never reaches that footer at all. **And a fourth time after a sixth round, which walked both edges of the hole the fifth had cut on purpose.** `breached = barred and not message and bool(output)` filed an escape *with* an error message as a refusal — telling the asking model nothing was read about material in that model's context — and filed a refusal reported in `output` *without* one as an escape, with a green card and a withdrawn assurance for a gate that had held. `StaticPolicy.MARK` is what tells them apart, in the one direction it is sound: present in either field means this app refused it, output without it means something ran, neither means this app cannot say. Its absence never reads as containment, so it can only strengthen a claim — the narrow use the two rounds that threw it out did not rule out. A breach is now `failed` like everything else the allowlist bars, because a green card for an escaped call is indefensible and the alarm above it already carries the difference. `failed` also stopped asking the vendor about a call that reported an error at all — `stopped` was barred-only, so on the master engine it reduced to `state == "ERROR"` and an error under `DONE` drew green with the reason discarded. **And the fourth category**: beside silence, falsehood and a stopped clock, the app being *certain*. A barred call that ends carrying neither the mark nor any output — rejected upstream, or killed between the request and the result — was written into the ungrounded list, so the header certified a clean policy refusal for a `SIGKILL`. There are three lists now and three sentences: refused, breached, and `unverified_tools` — *treat anything that looks retrieved as unverified rather than as absent* — with a `consultation_unverified` row on the tab, a warning rather than an error because what is missing is the certainty and not the containment. Measured live eight times (P21–P27): P25 arm E refuted the fourth round's claim that the bridge singulates (two barred tools, both named, `pending=0`), P26 arm H killed `agy` mid-call and caught the stranded spinner, P26 arm G and P27 arm I re-ran the cascade unchanged (arm I confirming the mark arrives on the live wire), and P27 arm J killed `agy` after prose so the header was composed for real: *"It reached for view_file, and ended without reporting what came back from that."* **And a fifth time after a seventh round, which found the test itself forgeable.** `StaticPolicy.MARK` is a fixed sentence written in this repository's own source, quoted in these specs and asserted in three test files — so *does the output contain the mark* is a **content** test, and the content is public. The failure it admits is the exact one the mechanism exists to catch: the gate fails open, a barred `view_file` reads `gate_server.py`, the output carries the mark because the mark is in that file, and a containment breach is filed as a denial with the alarm silent and the asking model told affirmatively that nothing was read. `StaticPolicy.stamped()` now mints `secrets.token_hex(8)` per consultation into `[ref <nonce>]` after the mark; `_run` stamps before the pump starts, `_is_our_refusal` matches the nonce alone, and an unstamped policy matches nothing and classifies every refusal `UNVERIFIED` — the honest answer, since an unauthenticated refusal *is* that state. A gate-side ledger was rejected first, on the reviewer's argument that a ledger records what the gate **ordered** and not what the engine **did**, so an engine that ignored a deny would reconcile clean. The round's second finding was the paragraph: P27 arm J's live header asserted *"nothing below was read, fetched or looked up"* and withdrew it two sentences later, so `_grounding` keeps the launch fact and drops the conclusion whenever anything is unaccounted for. The fixture had the same defect — it spelled the policy's reason, which a per-consultation nonce makes impossible to do honestly — and the fake `agy` now reads `hooks.json` from the root it was spawned against and runs the real `PreToolUse` command, so an offline refusal travels stamped policy → gate → socket → hook → wire → pump and the suite asserts a `[ref <16 hex>]` tag nothing in the test file wrote; which in turn exposed the allowed-tool test asserting an absence of notices against a consultation that had drawn no card at all. **And a sixth time after an eighth round, which broke the seventh's repair on the observation that a refusal is not a secret**: the reason exists to be *read* by the model, so a per-consultation token is published to the party it authenticates against by the first denial. A later gate failure plus any tool output quoting the conversation carries it home in `output`, and because `_is_our_refusal` runs before the output test, the escape is filed as a denial — the alarm disarmed by the gate's own words, in the one case where it matters. `Refusals` replaces the nonce: a token minted per **denial**, bound to the tool it was issued for, matched once and consumed, with the live set bounded at 64. The residue is stated rather than fixed — a *dropped* denial frame leaves one live token that an escape of the same tool could still reflect. **P28 then measured the three frames the round asked for and changed its other conclusion**: `tool_info.output` is **absent**, never empty, on a gate denial, on a schema rejection and on two tools that really ran and failed — and it is absent on a *completed* call too, so the proposed `output is not None` repair is safe and cannot close the hole it was aimed at. `state == "DONE"` with no `error` is the discriminator that works; the round's own suggestion, parser terminology, is refuted by `invalid_args` appearing verbatim in a genuine execution failure. A schema rejection is distinguishable instead by echoing **no `parameters`** and returning in 8 ms against 48–921 ms, and it reports **`DONE`** — so the invariant that a barred call which settles has failed whatever word `agy` put on it is load-bearing today, not defensive. **And a seventh time after a ninth and a tenth round, which converged and froze the design.** `output` is out of the classifier: `if output` was false for `""`, so a barred call that escaped and printed nothing was filed `UNVERIFIED` where the alarm belongs, and the proposed `output is not None` cannot fix it because `output` is *absent* rather than empty — including on a **completed** call. A barred call reaching `DONE` with **no `error` key** ran, and that is the breach test now; the key rather than the message because `agy` is Go and omits empty fields, so an error with an empty message would otherwise have read as a clean completion and manufactured an alarm out of a serialised default. **The blind spot is accepted and priced rather than closed**: a barred call that escaped and then failed in the host really executed, and is indistinguishable on the wire from one the vendor rejected before the gate — same barred tool, same terminal state, same foreign error, no token — so separating them needs the vendor-shaped heuristics every round has refused. Both stay `UNVERIFIED`, trading detection soundness for alarm precision, which costs the alarm and never the honesty, since `UNVERIFIED` already withdraws the header's conclusion. Two smaller holes went with it: a consultation that produced **no prose** handed the asking model "Antigravity returned an empty answer" while the tab showed `consultation_unverified` — the surface an agent is *blocked on* losing the containment event — and `_no_tools_or_fail` now refuses to launch under a policy permitting anything but `finish`, as a **raise rather than an `assert`**, since `assert` is stripped under `python -O`. |
...
| ~~An authenticated HTTP MCP listener~~ ✅ | [AG-22](decisions.md#ag-22), [AG-26](decisions.md#ag-26) | **Built 2026-09-12**, migration step 4, in `claude_code/consult_listener.py` — 33 tests, every one of them through a real MCP client against a real socket. A second listener on `127.0.0.1:0`, a `token → {repo_root, session_id}` map, a bearer gate in front of `/mcp`, and one `second_opinion` tool. Five of its properties were **measured rather than chosen** — statefulness is forced by `agy` reusing its session id across a **900-second idle gap** with zero traffic (P1) while never rebuilding a forgotten one; `ctx: Context` is stripped from the generated schema; the pre-bound socket does not leak into the `agy` child (P5); and a `notifications/cancelled` frame cancels the handler body while the original request still completes with a JSON-RPC error (P7b). What is **not** built is the wiring — the row below |
| ~~Wire the listener into `AgySession`, and write the per-spawn `mcp_config.json`~~ ✅ | [AG-22](decisions.md#ag-22), [AG-27](decisions.md#ag-27), [AG-R-25](risks.md#ag-r-25) | **Built 2026-09-12, closing [AG-1](decisions.md#ag-1)'s asymmetry** — see [AG-27](decisions.md#ag-27). `_offer_consultant` runs immediately before `AgySession` is constructed and `_retire_consultant` before it closes, so the credential's life is exactly the child's; it **fails soft**, since a second opinion is a tool and not a precondition. The allow-rule needed a new match **kind** rather than a value, because `call_mcp_tool` is a multiplexer like `run_command`: a rule keyed on the tool name would grant every tool on every server the user ever mounts from one click, so the value is the target and the dialog reads `call_mcp_tool(aic-dc-claude/second_opinion)`. Measured end to end on the real binary (P16): the canary sentence returned **verbatim** in 3.2 s with `permission_prompts=0`, the grant spending `1/2`, and the config gone from disk after shutdown; told to consult three times, `agy` made exactly three calls, reported the budget refusal verbatim and **did not retry**, ending `spent=2 attempts=2`. Building it found a fault the specification had no row for: `AgyService` constructed its `AntigravityPermissionGate` **without `config_dir`**, so standing rules went to `~/.config/aic-dc` while everything else the service owns lived where the host put it — it surfaced as a probe hung `11:31` on a dialog nobody could answer. Originally specified as: Mint at spawn and write the config into [AG-21](decisions.md#ag-21)'s private root; `begin_turn`/`end_turn` at the turn-push site, which is the only place that knows our own turn identity ([AG-R-24](risks.md#ag-r-24)); `cancel_session` from the ⏹ path so a stop reaches an in-flight consultation and not merely the master; `revoke` **and** `sweep_terminated` on subprocess exit and from `__aexit__`, because the SDK reclaims a transport in exactly one place and it is not the one that runs (AG-R-25). Until this exists the listener is reachable by nothing, so **the half of [AG-1](decisions.md#ag-1) that has never existed is still closed only on paper**. Also needs the allow-rule — `mcp(aic-dc-claude/second_opinion)`. **Three hazards found by reading the host, 2026-09-12.** *(1)* The master's root is deliberately **stable** so `--conversation` resume keeps working, so a bearer written to `<root>/.gemini/config/mcp_config.json` outlives the process at a predictable path — it must be written `0600` inside a `0700` directory (any local user can otherwise read a live token and call the tool), written **atomically and before `exec`** rather than blanked on exit, since `SIGKILL` skips exit hooks and every start must assume the file is stale. *(2)* Discovery is a **filesystem read through this app's own gate**: measured in both D1 arms, `agy` runs `list_dir` on `<root>/.gemini/antigravity-cli/mcp/<server>` and `view_file` on `<tool>.json` before it will call anything, so the gate must admit exactly that subtree read-only, by `realpath` prefix — and must **not** widen to `<root>/.gemini/`, which would hand the model `view_file` on its own bearer. Denied, this fails as *“the tool is not available”* rather than as a refused read. *(3)* `begin_turn` must **unconditionally** establish the budget for a new `turn_id`, so a missed `end_turn` can never leave a permanently spent grant; `end_turn` stays, in a `finally`, as the between-turns gate that stops a straggler consulting while `agy` sits idle |
| ~~A Claude consultant, so [AG-1](decisions.md#ag-1)'s asymmetry can close~~ ✅ | [AG-25](decisions.md#ag-25) | **Built 2026-09-12**, migration step 2, in `claude_code/consultant.py`. `claude_agent_sdk.query()` with `tools=[]` — **not** `allowed_tools=[]`, which emits no flag and gates nothing — `setting_sources=[]`, and a sterile working directory outside any repository tree. The two isolation mechanisms are independent on purpose: `setting_sources` is documented to govern settings files and `CLAUDE.md` and says nothing about the git environment block, so its suppression of that block is undocumented behaviour on one SDK version. The tripwire asserts on the **captured outbound request body**, not on the model's prose, because asking a model whether it saw a nonce uses a stochastic oracle as the assertion harness. Mutation-tested: swapping in `allowed_tools=[]` and `setting_sources=None` makes it fail with the `CLAUDE.md` nonce in the payload and 58 tools declared. Reaching it from an `agy` master is the row above; rendering it inline is still step 3 |
| ~~Honour `agy`'s cancellation notification on the MCP listener~~ ✅ | [AG-24](decisions.md#ag-24), [AG-26](decisions.md#ag-26) | **Built 2026-09-12 with the listener.** The handler never swallows `CancelledError`, so the runaway-spend failure cannot recur; the inner consultation is `ensure_future`'d and cancelled explicitly, because cancelling the handler does not touch it; and the quota is **refunded**, since a cancellation must never be answered with a success. Two traps came out of building it. The cancelled branch may **not await** — once a cancel scope has fired the next `await` re-raises before its body runs, so the first cut's cleanup silently never happened. And the obvious test method is a mirage: `read_timeout_seconds` on the client **times out locally and sends nothing on the wire**, so the test drives a raw `notifications/cancelled` frame, which is what `agy` actually sends at its 3m0s deadline | `agy` gives up at **3m0s** (measured exactly) and POSTs a cancellation the server must map onto the invocation's cancellation token. The scratch server ignored it and the tool ran to completion — which is the *same* runaway-spend failure the `run_command` design was rejected for, on the transport that replaced it. Fires on the **explicit notification only, never on a socket drop**: a loopback reset is not an intentional cancellation, and treating it as one rebuilds the sink-detachment defect already fixed here. The original request must still complete with a valid JSON-RPC error |
| ~~A per-turn consultation quota, keyed on **our** dispatched turn~~ ✅ | [AG-R-24](risks.md#ag-r-24), [AG-26](decisions.md#ag-26) | **Built 2026-09-12 in the listener**, as an atomic check-and-set under a lock **per grant** rather than per listener, so two sessions cannot serialise against each other. Keyed on our own `turn_id`, so nothing is inferred from `invocationNum` or `stepIdx` and the `(ServerName, ToolName)` match is unnecessary — the tool is ours and arrives as itself. **Two changes from the specification, both deliberate:** the budget is **two, not one**, because one consultation cannot be discussed and this app's own standing instruction is to converge over rounds; and a *separate* never-refunded attempt counter bounds retries, since a refund on failure otherwise gives an unbounded retry loop. Spending it returns **prose that tells the model to proceed**, since measured (E1) it is the prose and not the `isError` flag that decides whether `agy` retries — one tool call in each arm of a control. The "deny subagents" clause was recorded as **unbuildable** and then measured otherwise: a delegate reaches the same session id over the same bearer with the same headers, but its `params._meta` carries `antigravity.google/parent_conversation_id` and the master's does not, so a subagent is now refused before the budget check and spends neither an answer nor an attempt ([AG-26](decisions.md#ag-26)) | Every turn-boundary signal `agy` offers is misleading — `invocationNum` resets *within* a turn so keying on it fails open, and `stepIdx` never rolls back so a rollback fallback is dead code. The gate is our own in-process server inside the session that dispatched the turn, so the turn identity is ours and needs no inference. Must match on `(args.ServerName, args.ToolName)` because **every** MCP call arrives as `call_mcp_tool`; must be an atomic check-and-set, because a model can emit parallel calls in one step; and must deny subagents rather than let them spend the master's quota |
| ~~Admit `agy`'s MCP schema directory to the gate's reads by policy~~ ✅ | [AG-24](decisions.md#ag-24) § *`agy` does not name MCP tools* | **Built 2026-09-14.** Every MCP call is **two** hook events: a `view_file` of `…/antigravity-cli/mcp/<server>/<tool>.json` at one `stepIdx`, then the call at the next. `tools.py` classifies `view_file` as `read` with no path scoping, so this worked by accident — and the day a policy scopes reads, MCP discovery breaks as *"the model didn't call the tool"* rather than as a denial anyone can see. `roots.mcp_schema_dir` names the directory, `AgyGateServer` takes a `config_root`, and `decide()` admits a `view_file` resolved inside it ahead of both verdict paths. **Scoped to `antigravity-cli/mcp`, never the vendor root**, because `<root>/.gemini/config/mcp_config.json` holds the consultation listener's bearer — the hazard row 702 raised and this is where it is enforced. The path is **resolved before** the containment test, so `…/mcp/../config/mcp_config.json` does not pass, and `strict=False`, because `agy` writes these files during startup and a read can race their creation. **A consultation is given no `config_root` at all**: it is offered no MCP server, and a read granted around the static policy would falsify the header the asking model is handed with every test still green. It changes **no behaviour today**, and the row is honest about that rather than claiming a fix — today's denied-reads are repo-relative shift-clicks that cannot name a path inside the config root, so this is a tripwire for a narrowing that has not happened yet. Staged in the tests by pointing the user's own denied-reads list at the master root, which is the narrowest way to make that future real; against the shipped source the schema read comes back `deny`. Building it moved one thing: `_ensure_session` resolves the config root **before** it builds the gate server rather than after, since the server now takes it. 14 tests across four files. See [§ The two silences at the gate](delivery.md#the-two-silences-at-the-gate-2026-09-14) |
| ~~Nest a consultation's row under the tool call that spawned it~~ ✅ | [AG-28](decisions.md#ag-28), [AG-R-28](risks.md#ag-r-28) | **Built 2026-09-12**, and it is migration step 3's prerequisite rather than part of its surface: a consultation cannot be rendered *inline* until it has somewhere inline to be. An in-process MCP handler is given its own `args` dict and nothing else, so a decision-free `PreToolUse` hook — scoped to the two Antigravity tool names, returning `{}` on every path because a returned decision shadows `can_use_tool` — stages `(turn, tool, identifying-arguments) → tool_use_id` and the handler claims it back by content. `_claim` claims **only when exactly one staged entry matches**, which closes two holes at once: genuinely identical parallel calls, and a denied call whose staged id would otherwise be taken by a same-turn retry and render its live card under the call the user refused. Both fixes the review proposed for those were dead — an occurrence index *is* arrival-order pairing, and `PostToolUse` never fires for a call that never executed. `row.agent_id` stays **minted**: all three of its readers in the webapp are transcript *fetches*, and a consultation has no session on disk |
| ~~Render the consultation inline, with its posture, as the default surface~~ ✅ | [AG-29](decisions.md#ag-29), [AG-R-29](risks.md#ag-r-29) | **Built 2026-09-12.** `subagentBlocksExpanded` returns `true` for `task_type === 'consultation'` and `false` for every other subagent, because the collapse rule it breaks was written against a *delegated* turn — whose nested blocks are tool calls and whose result is a terse summary — and a consultation has no tool calls, so its stream **is** the answer. No double render: `blockExpanded` leaves the `second_opinion` card collapsed, so exactly one copy is on screen. The expansion check moved to `typeof === 'boolean'`, since against `=== true` an explicit collapse read as "unset" and the disclosure reopened itself. **The posture became container framing**, read from a severity-monotonic `agent_id` store (`noteConsultationPosture`) rather than authored from container identity — framing that could not retract would assert containment directly above the row saying it failed |
| ~~Stop creating a consultation's strip tab eagerly~~ ✅ | [AG-31](decisions.md#ag-31), [AG-R-31](risks.md#ag-r-31) | **Built 2026-09-12**, closing migration step 3. Reconciliation creates no consultation tab (`syncSubagentTab` delegates with `allowCreate = !isConsultation(row)`); the card's own name button does, through `openConsultationTab`. **Updating** an existing one is never gated, so it settles by the same `settleSubagentTab` every delegation uses rather than by a second lifecycle. One id space, `consultation:${id}`, because keying a running consultation and a finished one differently made the entity you got depend on the invisible turn boundary — the opener chooses its *source* (live `turnBlocks`, or the message the turn settled into) instead of the caller choosing a *kind*, which subsumes [AG-30](decisions.md#ag-30)'s `_projectSubagentBlocks` and its `historical:` tab. It **survives the send** and is swept only by a session change: its answer already renders inline, so the one reason to open the tab is to read it while writing the reply, and Send was destroying it exactly then. Review's prediction that a live-opened tab would orphan at turn end was refuted by probe — the mirror is the tab's own array and settling is in-memory — and this app's own plan to scope the strip ordinal per turn was refuted in turn, since a survivor would sit beside the next turn's first delegation both labelled `1`; consultations are not numbered at all |
| ~~Put the breach where a skimming reader will see it~~ ✅ | [AG-R-29](risks.md#ag-r-29) | **Built 2026-09-12.** The grounding paragraph is hoisted above the attribution when `escaped` or `unknown` is non-empty, so a surface that previews a result by its opening line leads with the withdrawal rather than with "a different model, reasoning independently — treat it as evidence". No rewording needed: it already opens *"Before the answer, from AIC⚡DC and not from the consultant"*. `reached` alone does **not** hoist, deliberately — a refusal is the gate *working*, and promoting the ordinary case leaves nothing for the one that matters. `is_error` is set on a breach only, which gives the card an `error` status and therefore an expanded body under `blockExpanded`, so a restored turn opens showing the retraction. Accepted cost: the asking model may retry, bounded by [AG-26](decisions.md#ag-26)'s per-turn quota |
| ~~Make the full streaming view a projection, not a disk read~~ ✅ | [AG-30](decisions.md#ag-30) | **Built 2026-09-12.** The row's button now goes to the tab it names instead of reporting *"that subagent is still active in the tab strip"* — which, because a consultation's tab is created eagerly and keyed by the same minted id, was not the edge case but every click. With no tab, the view is projected from the settled turn's blocks filtered to the spawning call's id, mounted in the shape `ensureFeedMessage` already gives a live subagent feed. The decision is a **server-sent fact**, `has_transcript: False`, absent meaning "has one" — not a capability check (a delegated subagent's blocks mirror by the same mechanism, so "has blocks" discriminates nothing) and not a task-type check in the view. One turn is exhaustive here because a consultation is awaited inside its tool call and cannot outlive its turn; a delegated subagent can, which is [AG-R-30](risks.md#ag-r-30). `bumpHistoricalGeneration` split out of `clearHistoricalTabs` so activating cancels an in-flight read without evicting anything |
| Repair a background subagent's head-truncated tab | [AG-R-30](risks.md#ag-r-30) | Found while building [AG-30](decisions.md#ag-30), in the path that change routes around. A background subagent's terminal event arrives in a later turn, creating a fresh tab holding only that turn's blocks; the tab is settled and non-empty, so `loadSubagentFeedIfEmpty`'s `blocks.length > 0` guard aborts the disk read and the user is shown the end of the work as though it were the whole. Head-truncation reads as complete, which is why it is worse than the tail kind. The real fix is the spec's own *Known gap* — routing each message to the translator that owns it — so it belongs to that change, not to this one |
| Give a consultation's tab a way to be closed | [AG-R-31](risks.md#ag-r-31) | Left by [AG-31](decisions.md#ag-31). A consultation's tab now survives every send and is swept only by a session change, which makes it the first tab whose lifetime the user cannot end — `tabs.js:264` records that `onTabClose` was deliberately removed because "both kinds of tab that remain sweep themselves", and that is no longer true of all three kinds. Bounded (one per consultation, and only on a click) but unthinnable. Neither obvious fix is small: restoring `onTabClose` for one kind re-opens a tab-model question, and sweeping at the `clearHistoricalTabs` cadence would close the first second opinion the moment a second is opened |
| Let a consultation asked *by* a subagent open its own tab | [AG-R-31](risks.md#ag-r-31) | Also from [AG-31](decisions.md#ag-31). `findSubagentBlocks` inherited `mirrorSubagentBlocks`'s refusal of a subagent owner, where it is correct — a mirror that read from one would re-mirror its own blocks — but a *read* cannot self-feed. Reachable: `allowed_tools` is deliberately never set and the shipped agent types carry `Tools: *`, so a Task subagent can call `second_opinion` and its card renders in that subagent's tab. Not fixed blind, because whether a delegation's disk-read feed carries the `agent_id` stamp the lookup matches on is unmeasured — if it does not, dropping the guard turns a blank tab into a blank tab |
| ~~Key the consultant's live conversation by consultation id~~ ✅ | [`blank-sheet-architecture.md`](../7-future/blank-sheet-architecture.md) step 3 | **Built 2026-09-14, closing migration step 3's last piece.** A `_Flight` per consultation on both transports, keyed by the `task_id` the bridge minted for the row, threaded through `stop_task` → `bridge.cancel` → the consultant. **The row was right about the shape and understated the damage by two.** `stop_task` already had the row's id in hand and *dropped* it, so ⏹ was aimed and then unaimed one call from its target; and the shared slot was not the only shared state. `AgyConsultant` also kept one `_cancelled` flag, read by every run, so stopping one consultation made a sibling that got its answer report *"stopped before it answered"* — the worse defect, because it destroys a paid answer rather than failing to stop one. The SDK consultant kept one `_last_connection` for the same reason, so a timing-out consultation quoted a *working* harness's stderr in its diagnosis. The id arrives as an **argument** rather than off the observer, because no sink here is load-bearing and a consultation with no browser attached must still be stoppable; a caller with no row gets a minted `local-` key, so two unnamed consultations cannot collide at one entry either. The trap is inside `_tab`, where three ids are in scope: `scope` is [AG-28](decisions.md#ag-28)'s borrowed `toolu_` that every block is stamped with, and `task_id` is the only one ⏹ ever sends. Two in flight is reachable rather than theoretical — the same `Tools: *` on shipped agent types that [AG-R-31](risks.md#ag-r-31)'s open row records from the render side. 17 tests, and the ones that can be stated in the shipped vocabulary were run against it: two harnesses and the wrong stderr quoted, and `cancel()` closing the younger process while the elder held. See [§ The stop that was aimed and then unaimed](delivery.md#the-stop-that-was-aimed-and-then-unaimed-2026-09-14) |
| ~~Serve the six `aic-dc` repo tools on both Antigravity transports~~ ✅ | [AG-34](decisions.md#ag-34), [AG-R-33](risks.md#ag-r-33), [AG-R-34](risks.md#ag-r-34) | **Built 2026-09-15, and the row it came from named one tool of six.** [AG-R-33](risks.md#ag-r-33) asked for a `ui_state`; `grep -rl` on the bridge's importers returned exactly one file, so `symbol_map`, `file_symbols`, `find_references`, `doc_outline` and `review_state` were absent by the same mechanism and had been the whole time — and **two records said otherwise**, `README.md`'s engine-agnostic bullet and `surface.PENDING_CONFIG["mcp_servers"]`, which described [AG-4](decisions.md#ag-4)'s channel as though something fed it. Nothing did, for eleven days; `options.py:240` accepted a `tools` argument no caller ever passed, which is worse than an absent field because the absent one fails at the call site. One spec table (`index_tools.SPECS`), three packagings: `@tool` on Claude, `ToolWithSchema` callables on the SDK transport, a second MCP server on the `agy` listener — a subprocess cannot be handed a callable. **The SDK path had to be measured, not assumed**: a bare callable's declaration comes from its *signature*, and all six handlers share `**arguments`, so the obvious build advertises six tools with one empty schema between them; `ToolWithSchema` is the one branch taking a schema verbatim, where `__name__` is the name and **`__doc__` is the description** — there is no description argument anywhere on that path — and `normalize_schema` was measured returning all six of ours byte-unchanged, which is what makes the three transports *identical* rather than equivalent. **The trap that would have shipped**: `policies=[deny_all(), *allows]` names `BuiltinTools` members only, so the six would have been declared to the model and then refused, in the Go harness, with nothing in Python to show it — a wasted turn that reads as the tool being broken. Two narrowings rather than one, because `agy` carries the call as `call_mcp_tool` with a server name (`is_index_read` checks the pair) while the SDK transport has no server, so the gate is told which **bare names this session registered** — deliberately not defaulted to `TOOL_NAMES`, since on `agy` a bare `symbol_map` would be the binary's tool sharing our spelling. First in `pre_verdict`, before the class table and `ALWAYS_ASK`, or it would never fire at all. Where the work actually went: `_offer_consultant` started the listener **only when a Claude CLI existed**, so the obvious build would have given repo intelligence to precisely the installs that already had Claude and left an `agy`-only user — the likeliest subscription shape — with neither. 41 tests written against the shipped source and all 41 failing, the listener's over the socket rather than in-process, because the defect was a tool that existed in Python and did not exist to a model. Left open: [AG-R-34](risks.md#ag-r-34), `ui_state`'s description promising *"files ticked in the picker"* that no snapshot has ever carried — moved verbatim so the extraction stayed provably byte-exact, and **closed the next morning with the entry corrected on the way**: told plainly that you cannot tick files in the picker, the fault is not a snapshot omitting a field but a promise about a control CC-21 removed, so the cost is a model asking the user to tick some rather than reading an empty selection. The reusable half is that pinning prose as a literal could not have caught it — every other check in `test_index_tools.py` asks whether the description is *unchanged*, and it was: unchanged and wrong from the day it was written. See [§ Six tools that existed in Python and did not exist to a model](delivery.md#six-tools-that-existed-in-python-and-did-not-exist-to-a-model-2026-09-15) |
...
- **Two tracks, and the rule for crossing between them.** This list and
  ...
  **Both were discharged on 2026-09-11, in that order, and migration step 1 is complete.** The rule has
  ...
  order.** With both tracks clear, the next thing was
  [R-14](../plan/risks.md#r-14--two-refreshes-race-for-one-single-use-refresh-token)'s lock, which
  [AG-22](decisions.md#ag-22) needs in front of it — **built 2026-09-11**, which leaves
  [AG-22](decisions.md#ag-22) itself with nothing ahead of it.

...
| Detect a renamed control tool, instead of denying the consultant its own `finish` | [AG-R-22](risks.md#ag-r-22) | Raised by the fifth consultation round, unchallenged by the sixth through the tenth, and **not built**. The consultation allowlist bars what it does not recognise, which is the safe direction for every entry except the one it *permits*: if `agy` renamed or namespaced `finish`, the gate would refuse the consultant the tool it ends its turn with, and every consultation would run to the bridge's timeout with nothing to show for the tokens. Cheap to detect — the gate sees the name it is denying — and the question is what to do about it, since permitting an unknown control tool by name shape is exactly the reasoning the allowlist exists to refuse. |
...
| Admit `agy`'s MCP schema directory to the gate's reads by policy | [AG-24](decisions.md#ag-24) § *`agy` does not name MCP tools* | Every MCP call is **two** hook events: a `view_file` of `…/antigravity-cli/mcp/<server>/<tool>.json` at one `stepIdx`, then the call at the next. `tools.py` classifies `view_file` as `read` with no path scoping, so this works today by accident. The day a policy scopes reads to the workspace, MCP discovery breaks — and breaks as *"the model didn't call the tool"* rather than as a denial anyone can see |

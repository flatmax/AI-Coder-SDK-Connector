...
| Make the full streaming view a projection, not a disk read | [`blank-sheet-architecture.md`](../7-future/blank-sheet-architecture.md) step 3 | Activate the live tab when one exists instead of toasting; otherwise project the retained in-memory blocks into a read-only tab. **Never call `get_subagent_transcript` for a consultation id** — it names no session on disk, which is the same reason [AG-28](decisions.md#ag-28) keeps `row.agent_id` minted — and do not offer the view at all on a turn restored from disk |

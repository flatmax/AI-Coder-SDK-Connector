...
| Key the consultant's live conversation by consultation id | [`blank-sheet-architecture.md`](../7-future/blank-sheet-architecture.md) step 3 | `consultant.py` holds a single `self._conversation` slot, so two consultations in flight share one handle and ⏹ cannot reach the right one. Replace it with a dict keyed on the consultation id, threading that id through `bridge.cancel()` and `stop_task` |

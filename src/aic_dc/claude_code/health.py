...
2. **Is its version compatible?** The SDK enforces a floor by *logging a
   warning* and continuing. AIC-DC fails startup instead, because a
...
def user_settings_file() -> Path | None:
    ...
    :func:`aic_dc.claude_code.commit._one_shot_options`, the one caller.
    """
...
class EngineHealth:
    ...
    #: terminal launched AIC-DC — which for a desktop launch is nowhere. The

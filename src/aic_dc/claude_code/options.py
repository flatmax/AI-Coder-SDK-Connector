...
:mod:`aic_dc.claude_code.session` for one reason: this is the surface most
...
- **Some options are never set, deliberately.** ``allowed_tools`` would
  ...
  tools. ``agents`` would make subagent definitions AIC-DC-only instead of
...
# in AIC-DC therefore behaves like a session in the CLI in the same repo
...
# Options AIC-DC must never set, with the reason, so a future reader who
...
NEVER_SET = {
    ...
    "shared with the CLI rather than being AIC-DC-only",
}
...
# to edit ``greet.py`` in a repo at ``/tmp/aic-dc-live``, the agent tried to
...
# (our own prompt text would fork behaviour between AIC-DC and the CLI in a
...
# localhost single-user host AIC-DC is built for, 16 MiB of pending buffer
...
def file_checkpointing_available(session_store: Any) -> bool:
    ...
    - The mirror carries `.aic-dc/` history, resume-after-restart, the
...
def build_options(**kwargs: Any) -> ClaudeAgentOptions:
    ...
    if unknown:
        raise EngineStartupError(
            ...
            "re-read it and update aic_dc.claude_code.options "

...
- **Only localhost may answer.** ``can_use_tool`` authorises arbitrary
  ...
  in :class:`~aic_dc.claude_code.service.ClaudeCodeService` (it needs the
...
def classify_tool(tool_name: str) -> str:
    ...
    if server is not None:
        return "read" if server == AIC_DC_MCP_SERVER else "mcp"
...
DENY_NO_LOCALHOST_REASON = (
    "No local AIC-DC client was connected to answer this permission request within "
...
DENY_SHUTDOWN_REASON = (
    "The AIC-DC session shut down before this permission request was answered."
)
...
def derive_suggested_rules(
    ...
    # only rule we could derive for them is a bare tool grant, and AIC-DC never
...
def derive_suggested_mode(suggestions: Any) -> dict[str, Any] | None:
    ...
    Only ever what the CLI suggested. AIC-DC does not invent a mode switch:
...
def _load_local_settings(repo_root: Path | str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(
            f"{path} is not valid JSON ({exc}). Fix it by hand — AIC-DC will not "
...
class PermissionBroker:
    ...
    async def can_use_tool(
        ...
        if mcp_server_name(tool_name) == AIC_DC_MCP_SERVER:
        ...
        try:
            ...
            return PermissionResultDeny(
                message=(
                    f"AIC-DC could not render a permission dialog for {tool_name}, so "
                    f"the call was denied. This is an AIC-DC fault, not a refusal."
                ),
                interrupt=False,
            )
    ...
    def _normalise(
        ...
        if action not in _DECISION_ACTIONS:
            ...
            return (
                "deny",
                f"AIC-DC did not recognise the decision {action!r}, so the call was denied.",
                None,
                None,
            )

"""Commit and reset — the two git writes AIC⚡DC still performs itself.
...
:class:`~aic_dc.claude_code.session.EngineSession`: sending a diff to the
...
Both leave a line in ``.aic-dc/events.jsonl``. Neither belongs in the
engine's transcript — the CLI never hears about a commit — so this is the
half of history AIC⚡DC owns, interleaved back into the browsed conversation
...
from aic_dc.claude_code.events_log import commit_content, reset_content
from aic_dc.claude_code.health import user_settings_file
...
def _one_shot_options(service: ClaudeCodeService, prompt: str) -> Any:
    ...
    try:
        ...
        logger.error(
            ...
            "aic_dc.claude_code.commit._one_shot_options.",
            exc,
        )
        return None
...
async def reset_to_head(service: ClaudeCodeService) -> dict[str, Any]:
    ...
    action in AIC⚡DC that throws away work with no way back, so the list of

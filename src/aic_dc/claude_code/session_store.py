"""The repo-local session store — one transcript, mirrored into ``.aic-dc/``.

AIC-DC implements the SDK's ``SessionStore`` protocol so a copy of the
...
# under .aic-dc/ that does not rebuild.
#
...
class RepoSessionStore:
    ...
    ----------
    root:
        The ``sessions/`` directory, normally ``.aic-dc/sessions``.
    ...
    The ``<project_key>`` level exists even though AIC-DC is single-repo:

"""``.aic-dc/events.jsonl`` — the events the transcript never held.

A commit, a reset, entering review, a preset switch, a permission-mode
change: AIC-DC did these, not the model, and none of them appear in the
...
(:mod:`aic_dc.claude_code.history`). Both remain valid ``event`` values, so
...
EVENT_TYPES = frozenset(
    {
        ...
        # a wrong field name in `.aic-dc/` is what the browser shows until
...
class EventsLog:
    ...
    ----------
    path:
        The log file, normally ``.aic-dc/events.jsonl``. Created on first

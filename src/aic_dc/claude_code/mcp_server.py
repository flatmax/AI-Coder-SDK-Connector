"""The ``aic-dc`` in-process MCP server — AIC-DC's repo intelligence as tools.
...
- **Read-only, all six.** Nothing here mutates the repository, the engine
  or the UI. That is what lets ``classify_tool`` treat ``aic-dc`` calls as
...
- **Never stale.** An index-reading tool flushes any pending re-index
  before it answers (:mod:`aic_dc.claude_code.hooks`). An agent misled by
...
class McpBridge:
    ...
    async def flush(self) -> None:
        ...
        try:
            ...
            logger.warning("Pending re-index failed before an aic-dc tool read: %s", exc)
    ...
    async def review_state(self) -> dict[str, Any]:
        ...
        lines = ["A code review is active in AIC-DC.", ""]
...
def _scope(
    ...
    both ``src/aic_dc`` and ``src/aic_dc/claude_code/mcp`` do what a reader

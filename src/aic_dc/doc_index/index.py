...
- **Excluded directories match the symbol index** — ``.git``,
  ``.aic-dc`` (and its legacy names), ``node_modules``, ``__pycache__``,
...
from aic_dc.doc_index.extractors import EXTRACTORS, BaseDocExtractor
...
# :attr:`aic_dc.symbol_index.index._EXCLUDED_DIRS`. ``.aic-dc`` and
# ``.ac-dc4`` and ``.ac-dc`` are this project's working-state dirs —
# the current name plus the two it used before — and excluding them
# avoids indexing the sidecar JSON files we just wrote.
...
class DocIndex:
    ...
    sidecar directory at ``{root}/.aic-dc/doc_cache/``.

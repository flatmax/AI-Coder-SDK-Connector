...
under ``.aic-dc/doc_cache/`` on top of the in-memory mtime cache
that :class:`~aic_dc.base_cache.BaseCache` provides. Keyword
...
from aic_dc.config import _AIC_DC_DIR
...
# Sidecar directory name under the per-repo ``.aic-dc/`` working
...
#
# The parent directory name (``.aic-dc``) is imported from the
# config module rather than hardcoded here — ``.aic-dc`` belongs
# to the previous AIC-DC implementation that shares repositories
# with this one during the transition, and colliding on that
# name would corrupt both states. See config._AIC_DC_DIR.
...
class DocCache(BaseCache[DocOutline]):
    ...
    (sidecars live at ``{repo_root}/.aic-dc/doc_cache/``).
    Construct with ``repo_root=None`` for pure in-memory caching
    (matches :class:`~aic_dc.symbol_index.cache.SymbolCache`).
    ...
    def __init__(self, repo_root: Path | str | None = None) -> None:
        ...
        repo_root
            ...
            are persisted to ``{repo_root}/.aic-dc/doc_cache/``.
    ...
    def _sidecar_dir(self) -> Path | None:
        ...
        if self._repo_root is None:
            return None
        return self._repo_root / _AIC_DC_DIR / _SIDECAR_DIR_NAME
    ...
    def _heading_from_json(cls, data: dict[str, Any]) -> DocHeading:
        from aic_dc.doc_index.models import DocSectionRef

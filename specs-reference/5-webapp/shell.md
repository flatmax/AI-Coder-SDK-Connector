...
| ~~`aic-dc-deny-read-scope`~~ | — | **Declined 2026-08-29 and will never exist.** The file picker's denial-scope prompt is decided against, not deferred: every deny rule goes to `.claude/settings.local.json` unconditionally. See `specs5/5-webapp/file-picker.md` § *Denial Scope Prompt — declined* and `specs5/next.md` § E |
